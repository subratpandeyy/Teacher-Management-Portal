...
export default function StudentInbox() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`student_broadcasts:${userId}:${id}`)
      .on(
        'postgres_changes',
        {
          event: 'INSERT',
      ...
      .subscribe((status) => {
        console.log(`Channel student_broadcasts:${userId}:${id} status:`, status);
      });

    return () => {
      console.log(`Removing channel student_broadcasts:${userId}:${id}`);
